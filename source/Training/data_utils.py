...
skip_words.add('man')
skip_words.add('men')
skip_words.add('woman')
skip_words.add('women')
skip_words.add('group')
skip_words.add('person')
skip_words.add('people')
...
def handle_words_gpt2(span, tokenizer, keywords=None, is_start=False):
    inputs = []
    labels = []
    words = nltk.word_tokenize(span)
    for w_i, w in enumerate(words):
        ...
        if keywords != None:
            if w in keywords:
                ...
                labels.extend([50257] * len(w_bpes))
        else:
            if w not in PERSON_NAMES and w not in skip_words and w.lower() not in skip_words:
                labels.extend(w_bpes)
            else:
...
def handle_underscores_gpt2(suffix, tokenizer, keywords=None, prefix=False):
    inputs = []
    labels = []
    if '_' in suffix:
        suffix_parts = [i.strip() for i in suffix.split('___')]
        for i, part in enumerate(suffix_parts):
            if part:
                tmp_inputs, tmp_labels = handle_words_gpt2(part, tokenizer, keywords=keywords, is_start=(i == 0 and prefix))
                inputs += tmp_inputs
                labels += tmp_labels
        ...
        inputs, labels = handle_words_gpt2(suffix, tokenizer, keywords=keywords, is_start=prefix)
    return inputs, labels


def convert_examples_to_features_gpt2(examples, tokenizer, max_length=512):
    data = []
    for example in examples:
        inputs, labels = handle_underscores_gpt2(example['context'], tokenizer, keywords=example['keywords'], prefix=True)
        choices = [handle_underscores_gpt2(cand, tokenizer) for cand in example['candidates']]
        input_ids = [inputs + cand[0] for cand in choices]
        input_ids = [tokenizer.convert_tokens_to_ids(cand) for cand in input_ids]
        label_ids = [labels + cand[1] for cand in choices]
        label_ids = [[t if t == 50257 else input_ids[i][t_i] for t_i, t in enumerate(cand)] for i, cand in
                     enumerate(label_ids)]
        label_ids = [[50257] + cand + [50257] for cand in label_ids]
        input_ids = [tokenizer.prepare_for_model(cand, max_length=max_length, truncation=True)['input_ids'] for cand in
                     input_ids]
        data.append([input_ids, label_ids, example['correct']])
    return data
...
class ATOMICProcessor(object):
    ...
    def load_data(self, filename, sid):
        with open(filename, "r") as f:
            for row in tqdm(f):
                sample = json.loads(row)
                tmp = sample['context'].split('. ')
                try:
                    if tmp[-1].strip() != '':
                        sample['question_only'] = tmp[-1]
                    else:
                        sample['question_only'] = tmp[-2]
                except:
                    print('You suck')
                    print(sample['context'].split('. '))
                    sample['question_only'] = sample['context']
