...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data, pad_token, mask_token, max_words_to_mask, evaluate):
        self.data = data
        self.pad_token = pad_token
        self.mask_token = mask_token
        self.max_words_to_mask = max_words_to_mask
        self.img_path = '/home/user16/HT/visually-augmented/dev_SyntheticQA' if evaluate else '/home/user16/HT/visually-augmented/train_SyntheticQA'
    ...
    def __getitem__(self, idx):
        ...
        img_fn = os.path.join(self.img_path, f'{idx}.pkl')
        with open(img_fn, 'rb') as f:
            img = pickle.load(f)

        return sample, self.pad_token, self.mask_token, self.max_words_to_mask, img['pooled_output_all']
...
def mCollateFn(batch):
    ...
    image_features = [b[-1] for b in batch] ###
    ...
    for index_of_features, f in enumerate(features): 
        batch_input_ids.append([])
        batch_input_mask.append([])
        batch_input_labels.append([])
        batch_label_ids.append(f[2])

        sequence_context_only = f[-2] + [pad_token] * (max_len - len(f[-2]))
        for i in range(len(f[0])):
            ...
            sequence_question_only = f[-1][i] + [pad_token] * (max_len - len(f[-1][i]))

            if len(sequence) != len(label_sequence):
                label_sequence = label_sequence[:len(sequence)-1] + [-100]
            ...
            batch_images_features.append(image_features[index_of_features])
            ...
            # input_ids_for_itm.append(sequence_question_only)
            # input_masks_for_itm.append([1] * len(f[-1][i]) + [0] * (max_len - len(f[-1][i])))
            input_ids_for_itm.append(sequence)
            input_masks_for_itm.append([1] * len(f[0][i]) + [0] * (max_len - len(f[0][i])))

        input_ids_for_hf.append(sequence_context_only)
        input_masks_for_hf.append([1] * len(f[-2]) + [0] * (max_len - len(f[-2])))
    return batch_input_ids, batch_input_mask, batch_input_labels, torch.tensor(batch_label_ids, dtype=torch.long), torch.tensor(input_ids_for_hf), torch.tensor(input_masks_for_hf), torch.tensor(input_ids_for_itm), torch.tensor(input_masks_for_itm), torch.stack(batch_images_features)
...
def train(args, train_dataset, model, tokenizer, eval_dataset):
    ...
    # loss_fct_for_prop = torch.nn.MultiMarginLoss(margin=0.5)
    bce_fct = torch.nn.BCEWithLogitsLoss()
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            image_hidden_state = batch[-1].to(args.device)

            input_ids = torch.cat([input_ids, batch[-3]]).to(args.device)
            att_mask = torch.cat([att_mask, batch[-2]]).to(args.device)

            # input_ids = torch.cat([input_ids, batch[-5]]).to(args.device)
            # att_mask = torch.cat([att_mask, batch[-4]]).to(args.device)

            inputs = {'input_ids': input_ids,
                      'attention_mask': att_mask,
                      'image_hidden_states': image_hidden_state}
            
            model.active_adapters = adapters.composition.BatchSplit("mlm_expert", "itm_expert",
                                                                    batch_sizes=[len(input_ids)-len(image_hidden_state), len(image_hidden_state)])

            outputs = model(**inputs)

            itm_score = outputs[-1]

            if outputs[0].size(0) == 0: continue
            ...
            for c_i in range(len(choice_seq_lens) - 1):
                ...
                choice_loss.append(-ce_loss[start:end].sum() / (end - start + 1e-9))

            choice_loss = torch.stack(choice_loss)
            choice_loss = choice_loss.view(-1, num_cand)

            itm = itm_score.view(-1, num_cand)
            
            loss1 = loss_fct(choice_loss, batch[3].to(args.device))
            loss2 = loss_fct(itm, batch[3].to(args.device))
            
            combined = (choice_loss + itm) / 2
            
            loss3 = loss_fct(combined, batch[3].to(args.device))
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    tb_writer.add_scalar('loss_mlm', loss1, global_step)
                    tb_writer.add_scalar('loss_itm', loss2, global_step)
                    tb_writer.add_scalar('loss_combine', loss3, global_step)
                ...
                if args.local_rank == -1 and args.evaluate_during_training and global_step % args.save_steps == 0:
                    ...
                    if results['acc'] > curr_best:
                        ...
                        ###
                        output_dir_adapter_mlm = args.output_dir+'/adapter_mlm'
                        output_dir_adapter_itm = args.output_dir+'/adapter_itm'
                        if not os.path.exists(output_dir_adapter_mlm):
                            os.makedirs(output_dir_adapter_mlm)
                        if not os.path.exists(output_dir_adapter_itm):
                            os.makedirs(output_dir_adapter_itm)
                        model_to_save = model.module if hasattr(model,
                                                                'module') else model  # Take care of distributed/parallel training
                        model_to_save.save_pretrained(output_dir)
                        ###
                        model_to_save.save_adapter(output_dir_adapter_mlm, 'mlm_expert')
                        model_to_save.save_adapter(output_dir_adapter_itm, 'itm_expert')
...
def evaluate(args, model, tokenizer, eval_dataset):
    ...
    for i, batch in enumerate(tqdm(eval_dataloader, desc="Evaluating")):
        ...
        preds_text.append(choice_loss)
        
        preds_image.append(itm)
        
        loss = (choice_loss + itm) / 2
        
        preds.append(loss)

    preds_text = torch.cat(preds_text, dim=0).cpu().numpy()
    preds_text = np.argmax(preds_text, axis=1)
    result_text = accuracy(preds_text, np.concatenate(out_label_ids, axis=0))

    preds_image = torch.cat(preds_image, dim=0).cpu().numpy()
    preds_image = np.argmax(preds_image, axis=1)
    result_image = accuracy(preds_image, np.concatenate(out_label_ids, axis=0))
    ...
    with open(output_eval_file, "w") as writer:
        logger.info("***** Eval results (Text) *****")
        for key in sorted(result_text.keys()):
            print("%s = %s\n" % (key, str(result_text[key])))
            logger.info("  %s = %s", key, str(result_text[key]))
            writer.write("%s = %s\n" % (key, str(result_text[key])))
        logger.info("***** Eval results (Image) *****")
        for key in sorted(result_image.keys()):
            print("%s = %s\n" % (key, str(result_image[key])))
            logger.info("  %s = %s", key, str(result_image[key]))
            writer.write("%s = %s\n" % (key, str(result_image[key])))
        logger.info("***** Eval results (Multi-modal) *****")
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    return MyDataset(features, tokenizer.pad_token_id, tokenizer.mask_token_id, args.max_words_to_mask, evaluate)
...
def main():
    ...
    if args.do_train:
        ...
        os.system("cp run_pretrain_adapter_image_v5.py %s" % os.path.join(args.output_dir, 'run_pretrain_adapter_image_v5.py'))
    ...
    model = model_class.from_pretrained(args.model_name_or_path, cache_dir=args.cache_dir)

    count = count_parameters(model)
    print(f'Before adding adapters: {count}\n')

    ### add adapter
    adapters.init(model) # prepare model for use with adapters

    config = adapters.ParBnConfig(reduction_factor=16)
    # config = adapters.AdapterConfig.load("lora", r=16) 
    model.add_adapter("mlm_expert", config=config)
    model.add_adapter("itm_expert", config=config)

    count = count_parameters(model)
    print(f'After adding adapters: {count}\n')
    model.train_adapter(["mlm_expert", 'itm_expert'])
