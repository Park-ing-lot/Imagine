...
You need to modify the modeling_gpt2.py and the corresponding __init__.py files in the Transformers package to correctly load adapters.
...
class GPT2Pooler_Imagine(nn.Module):
...
class GPT2Pooler_Imagine_image(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(1024, config.n_embd)
        self.activation = nn.Tanh()
        self.dropout = nn.Dropout(0.1)
...
class GPT2LMHeadModel_Imagine(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.itm_poller_image = GPT2Pooler_Imagine_image(config)
        self.itm_pooler = GPT2Pooler_Imagine(config)
    ...
    def forward(
        ...
        pooled_text = self.itm_pooler(pooled_logits[-len(image_hidden_states):, :])
        pooled_image = self.itm_poller_image(image_hidden_states)
        ...
        lm_logits = self.lm_head(hidden_states[:-len(image_hidden_states)])
            
        return (lm_logits,) + transformer_outputs[1:] + (qk, itm_scores,)
